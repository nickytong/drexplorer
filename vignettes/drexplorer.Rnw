...
There is a great variety of models developed for dose-response data, many of which have been implemented in 
 ...
 perform on the data. We also incorporate model selection. Another important feature for \texttt{drexplorer} is to allow the user to identify outlier measurements and visually examine
 how these outliers affect the fitted model. 

 In addition to fit dose-response models, \texttt{drexplorer} also implements methods previously published to assess drug-drug interaction. 
...
Two papers have been published regarding to drug interaction index (IAI) by Lee et al, 
	one in 2007 (Lee2007) \citep{lee2007interaction} and on in 2009 (Lee2009) \citep{lee2009confidence}. The Lee2007 paper 
...
of these models, please refer to Lee2007 \citep{lee2007interaction}.

The two papers by Lee et al discussed the fixed ratio design and the source code for doing this is incorporated
into \texttt{drexplorer}. To work on grid design, a fixed ratio from the data needs to be selected in order to apply their method. For
...
fixed ratio would affect the fitted median effect model (see definition in \citep{lee2007interaction}) for the drug mixture as well as
estimation of IAI. As a result, IAI has a ratio dependent interpretation. 

Below we load the UMSCC22B data from \citep{lee2009confidence}. This data has a fixed ratio design. The \texttt{fitIAI} function estimates IAI
...
drug mixture, see equation (6) in \citep{lee2007interaction}), median effect plot and dose response curves. We can also plot IAI versus response as well as IAI versus
dose in one figure by specifying mode='both'. 

The median effect equation \citep{chou1984quantitative} is as following:
...
In \citep{lee2007interaction}, there is an example data (nl22B2) using grid design. Here we examine the estimate of IAI at different fixed ratios.
