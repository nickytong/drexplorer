...
% \VignetteIndexEntry{An introduction to drexplorer}
...
library(knitr)
library(drexplorer)
opts_chunk$set(tidy=TRUE,tidy.opts=list(keep.blank.line=FALSE, width.cutoff=50),out.truncate=80,out.lines=6,cache=FALSE,dev='pdf',include=TRUE,fig.width=6,fig.height=6,resolution=150)
@

\newcommand{\fixme}[1]{{\textbf{Fixme:} \textit{\textcolor{blue}{#1}}}}

\title{Dose-response Explorer: Exploring different models and drug-drug interaction} 
...
There is a great variety of models developed for dose-response data, many of which have been implemented in 
 the \Rpackage{drc} and \Rpackage{DoseFinding} packages. \Rpackage{drexplorer} combines both packages to aid the user to visually examine and compare how existing models
 perform on the data. We also incorporate model selection. Another important feature for \Rpackage{drexplorer} is to allow the user to identify outlier measurements and visually examine
 how these outliers affect the fitted model. 

 In addition to fit dose-response models, \Rpackage{drexplorer} also implements methods previously published to assess drug-drug interaction. 
...
designed to allow users without advanced programming skills to perform dose-response analysis.  
 ...
 The main entry for \Rpackage{drexplorer} is the drFit() function and computeIC() function. drFit() fits a model.
...
We use the \texttt{ryegrass} data from \Rpackage{drc} package for illustration purpose.

First, we load the \Rpackage{drexplorer} package and attach the \Rpackage{ryegrass} data.
<<loadLibrary>>=
library(drexplorer)
data(ryegrass)
@ 
...
\section{Assessing Reproducibility}
...
Here we simulate two response vectors and calculate CCC. The computed CCC value is \Sexpr{round(ccc['ccc'], 3)}, location shift is
\Sexpr{round(ccc['l_shift'], 3)}, scale shift is \Sexpr{round(ccc['s_shift'], 3)}, Pearson correlation is \Sexpr{round(ccc['corr'], 3)}. 

\section{Fit dose-response models}
Below we show how to fit a dose-response model. The fitDRC() function is a wrapper to the \Rpackage{drc} and \Rpackage{DoseFinding} packages.
...
\section{Drug Interaction Index}
...
Two papers have been published regarding to drug interaction index (IAI) by Lee et al, 
	one in 2007 (Lee2007) \cite{lee2007interaction} and on in 2009 (Lee2009) \cite{lee2009confidence}. The Lee2007 paper 
...
of these models, please refer to Lee2007 \cite{lee2007interaction}.

The two papers by Lee et al discussed the fixed ratio design and the source code for doing this is incorporated
into \Rpackage{drexplorer}. To work on grid design, a fixed ratio from the data needs to be selected in order to apply their method. For
...
fixed ratio would affect the fitted median effect model (see definition in \cite{lee2007interaction}) for the drug mixture as well as
estimation of IAI. As a result, IAI has a ratio dependent interpretation. 

Below we load the UMSCC22B data from \cite{lee2009confidence}. This data has a fixed ratio design. The \texttt{fitIAI} function estimates IAI
...
The \texttt{plotIAI} function is then used to generate different plots including IAI versus response, IAI versus dose (predicted dose for the
drug mixture, see equation (6) in \cite{lee2007interaction}), median effect plot and dose response curves. We can also plot IAI versus response as well as IAI versus
dose in one figure by specifying mode='both'. 

The median effect equation \cite{chou1984quantitative} is as following:
...
In \cite{lee2007interaction}, there is an example data (nl22B2) using grid design. Here we examine the estimate of IAI at different fixed ratios.
...
\bibitem{lee2007interaction}
Lee, J Jack and Kong, Maiying and Ayers, Gregory D and Lotan, Reuben
\newblock Mapping and quantifying mammalian transcriptomes by rna-seq.
\newblock {\em Journal of biopharmaceutical statistics}, 17(3):461--480, 2007.

\bibitem{lee2009confidence}
Lee, J Jack and Kong, Maiying
\newblock Confidence intervals of interaction index for assessing multiple drug interaction.
\newblock {\em Statistics in biopharmaceutical research}, 1(1):4--17, 2009.

\bibitem{chou1984quantitative}
Chou, Ting-Chao and Talalay, Paul
\newblock Quantitative analysis of dose-effect relationships: the combined effects of multiple drugs or enzyme inhibitors.
\newblock {\em Advances in enzyme regulation}, 22:27--55, 1984.

\end{thebibliography}

\bibliographystyle{unsrt}
\bibliography{drexplorer}

\end{document}

%   cd /data/bioinfo2/ptong1/Projects/Coombes/IC50Package/Package/for_vignettes; R --vanilla
%   knitr::knit("drexplorer.Rnw")
